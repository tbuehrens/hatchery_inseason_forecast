...
test_model<-function(series,minyr,leaveout,leave_yrs,iter,Y,S,n){
  for(i in 1:iter){ 
    last_yr = max(series$year) - (iter-i+1)
    ...
    p<-ggplot(data=tdat,aes(x=date,y=abundance,group=year,color=factor(train_test)))+
      ...
      geom_point()+
      geom_ribbon(aes(ymin =`Lo 95`, ymax =`Hi 95`), color =NA,fill="grey",alpha=0.5)+
...
test_model2<-function(series,minyr,leaveout,leave_yrs,iter,Y,S,n){
  for(i in 1:iter){ 
    last_yr = max(series$year) - (iter-i+1)
    tdat<-series%>%
      mutate(train_test=ifelse(year > last_yr & yday > (S-leaveout), 1, 0),
...
results<-test_model(series=series,minyr=minyr,leaveout=leaveout,leave_yrs=leave_yrs,iter=iter,Y=Y,S=S,n=n)
results<-results%>%
...
results2<-test_model2(series=series,minyr=minyr,leaveout=leaveout,leave_yrs=leave_yrs,iter=iter,Y=Y,S=S,n=n)
results2<-results2%>%
  mutate(pct_error=(error/abundance)*100)
